    ...
    >
      ...
      >
        ...
        <div className="bg-gradient-to-b from-slate-950 to-slate-900 px-4 py-3 flex items-center justify-between">
          ...
          <div className="text-xs font-extrabold tracking-widest uppercase text-cyan-300">Profile</div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="px-4 pt-5 pb-4 flex items-start gap-4">
                {profile.photoURL ? (
                  <img src={profile.photoURL} alt={profile.name} className="w-16 h-16 rounded-full object-cover ring-2 ring-cyan-200" />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 text-white text-xl font-extrabold flex items-center justify-center">
                    {initials}
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-1.5 flex-wrap">
                    {profile.role === 'coach' && (
                      <span className="text-[10px] font-extrabold tracking-widest uppercase px-1.5 py-0.5 rounded bg-cyan-50 text-cyan-700 border border-cyan-200">
                  ...
                  {(showEmail || showPhone) && (
                    <div className="mt-2 space-y-0.5">
                      {showEmail && (
                        <a href={`mailto:${profile.email}`} className="block text-xs text-cyan-700 hover:text-cyan-900 truncate">
                          {profile.email}
                        </a>
                      )}
                      {showPhone && (
                        <a href={`tel:${profile.phoneNumber}`} className="block text-xs text-cyan-700 hover:text-cyan-900">
                          {profile.phoneNumber}
                        </a>
                      )}
                    </div>
              ...
              <div className="px-4 pb-3 grid grid-cols-2 gap-2">
                {onStartDm && (
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg bg-cyan-50 text-cyan-700 border border-cyan-200 text-xs font-extrabold tracking-widest uppercase hover:bg-cyan-100"
                  >
