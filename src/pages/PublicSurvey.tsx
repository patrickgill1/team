...
const PublicSurvey: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    // Validate required questions. Empty spans single-value ('' / undefined)
    // AND empty array (multi-select MC with nothing picked) — treated the
    // same way per design so required means "at least one pick".
    ...
    setSubmitting(true);
    try {
      // Prune empty answers from the submitted payload so a hidden-then-
      // shown or momentarily-touched question doesn't persist as junk.
      // Same "unanswered" definition as the required guard above.
