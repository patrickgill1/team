    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">Player of the Match</h1>
              <p className="text-gray-300 mt-1">Vote for outstanding performances</p>
            </div>
            {isUserCoach && (
              <button
            ...
            {availableGames.length > 0 ? (
              <div className="bg-cyan-500/10 border border-cyan-100 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-medium text-cyan-200 mb-2">
                    ...
                    <div className="space-y-2">
                      {availableGames.map(game => (
                        <div key={game.id} className="flex items-center justify-between bg-gray-900/80 rounded-lg p-3 border border-cyan-100">
                          <div>
              ...
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-amber-100 mb-1">No Games Scheduled</h3>
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-200 mb-1">All Recent Games Have Voting</h3>
                    <p className="text-sm text-gray-300">
        ...
        {activeVoting && (
          <div className="card-modern mb-6 overflow-hidden">
            <div className="px-6 py-4 border-b border-amber-100 bg-amber-500/10">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-amber-200">🏆 Active Voting</h2>
                  <p className="text-amber-300">{activeVoting.gameTitle} - {formatDate(activeVoting.gameDate)}</p>
                  {activeVoting.calendarEventId && (
                    <p className="text-sm text-amber-300/80">🔗 Linked to calendar event</p>
                  )}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className={`flex items-center space-x-2 px-4 py-2 rounded-xl font-medium transition-all duration-200 border ${
                      linkCopied
                        ? 'bg-emerald-500/10 border-emerald-200 text-emerald-300'
                        : 'bg-white border-amber-200 text-amber-800 hover:bg-amber-500/10'
                    }`}
              ...
              {usersVote ? (
                <div className="bg-emerald-500/10 border border-emerald-100 rounded-lg p-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <p className="font-medium text-emerald-200">You voted for {usersVote.playerName}</p>
                      {usersVote.reason && (
                ...
                <div className="text-center text-gray-300">
                  ...
                  {activeVoting.eligiblePlayerIds && activeVoting.eligiblePlayerIds.length > 0 && !isUserCoach && (
                    <p className="text-sm text-gray-400 mt-1">Only parents of players who were present at the match can vote.</p>
                  )}
                </div>
              )}
                      ...
                      return (
                        <div key={result.playerId}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-white w-36 truncate">{result.name}</span>
                            ...
                            <span className="text-sm text-gray-400 w-16 text-right">{result.count} · {pct}%</span>
                          </div>
                        </div>
                      );
                    })}
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-white/5 border-b border-white/10">
                          <tr>
                            ...
                            <th className="text-left px-3 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wide">Reason</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-white/10">
                          {activeVoting.votes.map((v, i) => (
                            <tr key={i} className="hover:bg-white/5">
                              <td className="px-3 py-2 font-medium text-white">{v.voterName || '—'}</td>
                              <td className="px-3 py-2 text-gray-200">{v.playerName}</td>
                              <td className="px-3 py-2 text-gray-400 italic">{v.reason || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {activeVoting.votes.length > 0 && !isUserCoach && (
                <div className="mt-6 pt-6 border-t border-white/10">
                  <div className="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
                    ...
                    <p className="text-sm font-medium text-gray-200">Results are hidden while voting is open</p>
                    <p className="text-xs text-gray-400 mt-1">Results will be visible once the coach closes voting</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="p-6">
            {votings.filter(v => !v.isActive).length === 0 ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-white mb-2">No completed votings</h3>
                <p className="text-gray-300">Previous Player of the Match results will appear here</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={voting.id} className="border border-white/10 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="font-semibold text-white">{voting.gameTitle}</h3>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <div>
                                {voting.winners && voting.winners.length > 1 ? (
                                  <>
                                    <p className="font-semibold text-amber-400 text-sm">Co-Players of the Match ({voting.winners.length})</p>
                                    <p className="text-xs text-gray-300">{voting.winners.map(w => w.playerName).join(', ')}</p>
                                    <p className="text-xs text-gray-400 mt-0.5">{voting.winners[0].voteCount} votes each</p>
                                  </>
                                ) : (
                                  <>
                                    <p className="font-semibold text-amber-400">{(voting.winners?.[0] || voting.winner)!.playerName}</p>
                                    <p className="text-sm text-gray-300">{(voting.winners?.[0] || voting.winner)!.voteCount} votes</p>
                                  </>
                                )}
                              </div>
                            </div>
      ...
      {showAttendanceStep && (pendingVotingData || editingVotingId) && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900/80 rounded-2xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-semibold text-white mb-2">{editingVotingId ? 'Edit Attendance' : 'Mark Attendance'}</h2>
            {pendingVotingData && (
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900/80 rounded-2xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-semibold text-white mb-4">Create Player of the Match Voting</h2>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {availableGames.map(game => (
                    <button
                      key={game.id}
                      onClick={() => handleCreateVotingFromCalendarEvent(game.id)}
                      className="w-full text-left p-3 border border-white/10 rounded-lg hover:border-cyan-300 hover:bg-cyan-500/10 transition-colors duration-200"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
            ...
            {availableGames.length === 0 && calendarEvents.filter(e => e.type === 'game').length === 0 && (
              <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-amber-100 mb-1">No Games in Calendar</h4>
                    <p className="text-sm text-amber-300">
                      To link Player of the Match voting to your games, first schedule games in your calendar:
                    </p>
                    <ol className="text-xs text-amber-400 mt-2 space-y-1 list-decimal list-inside">
...
const CustomGameForm: React.FC<CustomGameFormProps> = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-200 mb-1">Game Title</label>
        <input
          type="text"
          value={gameTitle}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-200 mb-1">Game Date</label>
          <input
            type="date"
            value={gameDate}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-200 mb-1">Game Time</label>
          <input
            type="time"
            value={gameTime}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-200 mb-1">Opponent <span className="text-gray-400 font-normal">(optional)</span></label>
          <input
            type="text"
            value={opponent}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-200 mb-1">Home / Away <span className="text-gray-400 font-normal">(optional)</span></label>
          <select
            value={homeAway}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-200 mb-1">Venue / Location <span className="text-gray-400 font-normal">(optional)</span></label>
        <input
          type="text"
          value={location}
