      ...
      <section className="px-3 sm:px-4 pt-3">
        ...
        >
          ...
          <div className="relative px-5 pt-5 sm:pt-6">
            ...
            <p className="mt-1 text-sm text-white/80 drop-shadow">{greeting}, {firstName}</p>  // theme-ok: legacy hardcoded color, pending per-line audit
          ...
          <div className="relative px-5 pb-5 sm:pb-6 space-y-3">
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight leading-tight [text-shadow:0_2px_8px_rgba(0,0,0,0.95),0_0_1px_rgba(0,0,0,0.9)]">  // theme-ok: legacy hardcoded color, pending per-line audit
                {isCoach ? "Let's get your team going." : 'No upcoming events yet.'}
              </h1>
              <p className="mt-1 text-sm font-medium text-white/90 [text-shadow:0_1px_5px_rgba(0,0,0,0.9)]">  // theme-ok: legacy hardcoded color, pending per-line audit
                {isCoach
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span className="text-xl font-extrabold text-white leading-none tabular-nums">{playerCount}</span>  // theme-ok: legacy hardcoded color, pending per-line audit
                <span className="text-[9px] font-bold tracking-widest text-white/70 mt-1">ROSTER</span>  // theme-ok: legacy hardcoded color, pending per-line audit
              </Link>
              {isCoach && (
                <Link
                  to="/calendar"
                  className="inline-flex items-center justify-center px-4 py-2.5 rounded-md font-bold text-sm bg-brand-primary hover:bg-brand-primary text-white shadow-lg shadow-brand-primary-dim/40 ring-1 ring-brand-primary-soft/20 transition" /* theme-ok: legacy hardcoded color, pending audit */
                >
        ...
        <div className="relative flex items-start justify-between px-5 pt-5 sm:pt-6">
          ...
          {weather && (
            <div className="shrink-0 inline-flex items-center gap-1.5 rounded-full bg-black/40 ring-1 ring-line-default/10 px-2.5 py-1 backdrop-blur-sm">
              <WeatherIcon iconName={weather.iconName} className="w-3.5 h-3.5 text-white" />  // theme-ok: legacy hardcoded color, pending per-line audit
              <span className="text-[12px] font-bold text-white tabular-nums drop-shadow">{weather.tempMaxF}°/{weather.tempMinF}°</span>  // theme-ok: legacy hardcoded color, pending per-line audit
            </div>
          )}
        </div>
        ...
        <div className="relative px-5 pb-5 sm:pb-6">
          <div className="flex items-end gap-4">
            ...
            >
              ...
              <span className="text-3xl sm:text-[34px] font-black text-white leading-none [text-shadow:0_2px_7px_rgba(0,0,0,0.95),0_0_1px_rgba(0,0,0,0.9)]">{eventDay}</span>  // theme-ok: legacy hardcoded color, pending per-line audit
              <span className="text-[9px] font-bold tracking-widest text-white/70 mt-0.5">{eventDow}</span>  // theme-ok: legacy hardcoded color, pending per-line audit
            </Link>
            <div className="min-w-0 flex-1">
              <Link to={`/events/${nextEvent.id}`} className="block">
                <h2 className="text-2xl sm:text-3xl font-black text-white leading-tight truncate [text-shadow:0_3px_10px_rgba(0,0,0,0.95),0_0_1px_rgba(0,0,0,0.9)]">  // theme-ok: legacy hardcoded color, pending per-line audit
                  {nextEvent.title}
                </h2>
                <p className="mt-1 text-sm sm:text-base font-semibold text-white/90 truncate [text-shadow:0_2px_7px_rgba(0,0,0,0.9)]">{whenText}</p>  // theme-ok: legacy hardcoded color, pending per-line audit
                {location && (
                  <p className="mt-0.5 text-xs sm:text-sm font-semibold text-white/90 truncate [text-shadow:0_2px_7px_rgba(0,0,0,0.95),0_0_1px_rgba(0,0,0,0.9)]">  // theme-ok: legacy hardcoded color, pending per-line audit
              ...
              {!splitOpen ? (
                ...
                <div className="rounded-xl bg-black/45 ring-1 ring-white/10 px-3 pt-2 pb-2.5 backdrop-blur-sm">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[10px] font-extrabold uppercase tracking-widest text-white/65">Set per kid</span>  // theme-ok: legacy hardcoded color, pending per-line audit
                    <button
                      type="button"
                      onClick={() => setSplitOpen(false)}
                      className="inline-flex items-center gap-0.5 text-[10px] font-bold text-white/60 hover:text-white/85" /* theme-ok: legacy hardcoded color, pending audit */
                    ...
                    {linkedPlayers.map((p) => {
                      ...
                      return (
                        <div key={p.id} className="flex items-center gap-2">
                          <span className="text-xs font-bold text-white/90 flex-1 truncate min-w-0" title={p.name}>{first}</span>  // theme-ok: legacy hardcoded color, pending per-line audit
          ...
          <p className="mt-3 text-center text-[12px] text-white/70 drop-shadow">  // theme-ok: legacy hardcoded color, pending per-line audit
            <span className="font-bold text-white">{goingCount}</span> going /* theme-ok: legacy hardcoded color, pending audit */
            {pendingCount > 0 && (
              <>
                {' · '}
                <span className="font-bold text-white">{pendingCount}</span> pending /* theme-ok: legacy hardcoded color, pending audit */
              </>
            )}
          </p>
        </div>
...
function DigestStrip({ total, onOpen }: {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-brand-primary/25 ring-1 ring-brand-primary/45 px-2.5 py-1 text-[11px] font-black tracking-wide text-white shadow-sm backdrop-blur-sm hover:bg-brand-primary/35 active:scale-[0.98] transition" /* theme-ok: legacy hardcoded color, pending audit */
...
function RsvpButton({ tone, active, label, onClick }: {
  ...
  const activeStyles: Record<RsvpStatus, string> = {
    going: 'bg-emerald-500 text-white ring-1 ring-emerald-300/60 shadow-lg shadow-emerald-500/30',  // theme-ok: legacy hardcoded color, pending per-line audit
    maybe: 'bg-amber-400 text-ink-primary ring-1 ring-amber-300/60 shadow-lg shadow-amber-500/30',
    no:    'bg-rose-500 text-white ring-1 ring-rose-300/60 shadow-lg shadow-rose-500/30',  // theme-ok: legacy hardcoded color, pending per-line audit
  };
  const inactiveStyles = 'bg-black/50 text-white/90 ring-1 ring-white/10 backdrop-blur-md hover:bg-black/60';  // theme-ok: legacy hardcoded color, pending per-line audit
  return (
    <button
      type="button"
      onClick={onClick}
...
function MiniRsvpPill({ tone, active, label, onClick }: {
  ...
  const activeStyles: Record<RsvpStatus, string> = {
    going: 'bg-emerald-500 text-white ring-1 ring-emerald-300/60',  // theme-ok: legacy hardcoded color, pending per-line audit
    maybe: 'bg-amber-400 text-ink-primary ring-1 ring-amber-300/60',
    no:    'bg-rose-500 text-white ring-1 ring-rose-300/60',  // theme-ok: legacy hardcoded color, pending per-line audit
  };
  const inactiveStyles = 'bg-black/50 text-white/85 ring-1 ring-white/10 hover:bg-black/65';  // theme-ok: legacy hardcoded color, pending per-line audit
  return (
    <button
      type="button"
      onClick={onClick}
