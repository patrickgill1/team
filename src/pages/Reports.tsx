      ...
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="flex items-end justify-between gap-3 flex-wrap">
          <div>
            ...
            <h1 className="text-2xl font-black text-ink-primary mt-1">Reports</h1>
            <p className="text-sm text-ink-primary/65">Funnel + fees + coupon usage. Filter by season.</p>
          </div>
          <select
            value={seasonFilter}
            onChange={(e) => setSeasonFilter(e.target.value)}
            className="text-sm border border-line-default/15 rounded-lg px-3 py-2"
          >
            ...
            <Section title="By age group">
              {byAge.length === 0
                ...
                : <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {byAge.map(([k, n]) => (
                      <div key={k} className="rounded-lg ring-1 ring-line-default/10 bg-line-default/[0.04] p-2 text-center">
                        <div className="text-lg font-black text-ink-primary">{n}</div>
                        <div className="text-[10px] font-extrabold uppercase tracking-widest text-ink-primary/50">{k}</div>
                      </div>
                    ))}
                  </div>}
            </Section>
                ...
                : <ul className="space-y-1">
                    {byStatus.map(([status, n]) => (
                      <li key={status} className="flex items-center justify-between text-sm">
                        <span className="text-ink-primary/85">{status}</span>
                        <span className="font-bold text-ink-primary tabular-nums">{n}</span>
                      </li>
                    ))}
                  </ul>}
            </Section>
                ...
                : <ul className="space-y-1">
                    {coupons.map(([code, info]) => (
                      <li key={code} className="flex items-center justify-between text-sm">
                        <span className="font-bold text-ink-primary tracking-wider">{code}</span>
                        <span className="text-ink-primary/65 tabular-nums">{info.count}× · {fmtCents(info.discountCents)} off</span>
                      </li>
                    ))}
                  </ul>}
            </Section>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="bg-surface-elevated rounded-2xl ring-1 ring-line-default/10 overflow-hidden">
    <div className="px-4 py-2 border-b border-line-default/5">
      <h2 className="text-[10px] font-extrabold uppercase tracking-widest text-ink-primary/65">{title}</h2>
    </div>
  ...
  return (
    <div className="mb-2 last:mb-0">
      <div className="flex items-baseline justify-between text-xs">
        <span className="font-bold text-ink-primary/85">{label}</span>
        <span className="text-ink-primary/65 tabular-nums">{value} <span className="text-ink-primary/40">({pct}%)</span></span>
      </div>
      <div className="h-2 rounded-full bg-surface-base overflow-hidden mt-1">
...
const Empty: React.FC<{ text: string }> = ({ text }) => (
  <p className="text-xs text-ink-primary/50">{text}</p>
);
