...
export function seasonLifecycleTone(s: SeasonLifecycle): { bg: string; text: string; ring: string } {
  switch (s) {
    case 'draft': return { bg: 'bg-slate-100', text: 'text-ink-primary/90', ring: 'ring-slate-300' };
    ...
    case 'ended': return { bg: 'bg-slate-100', text: 'text-ink-primary/80', ring: 'ring-slate-300' };
    case 'archived': return { bg: 'bg-slate-100', text: 'text-ink-primary/65', ring: 'ring-slate-200' };
  }
}
