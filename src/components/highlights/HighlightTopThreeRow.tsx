...
function rankStyle(rank: 1 | 2 | 3): { color: string; glow: string; label: string } {
  if (rank === 1) {
    return {
      ...
      glow: 'drop-shadow-[0_2px_12px_rgba(251,191,36,0.55)]',
      label: 'Gold',
    };
  }
  if (rank === 2) {
    return {
      ...
      glow: 'drop-shadow-[0_2px_10px_rgba(226,232,240,0.35)]',
      label: 'Silver',
    };
  }
  return {
    color: 'text-orange-400',
    glow: 'drop-shadow-[0_2px_10px_rgba(251,146,60,0.45)]',
    label: 'Bronze',
  };
}
...
const TopCard: React.FC<TopCardProps> = ({ rank, clip, players, onTap }) => {
  ...
  return (
    <button
      type="button"
      onClick={onTap}
      className="group relative w-[300px] md:w-[380px] snap-start shrink-0 aspect-video rounded-2xl overflow-hidden bg-surface-elevated ring-1 ring-line-default/10 text-left transition-transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-brand-primary/60"
    ...
    >
      ...
      {/* Full-height gradient so the numeral + label area always has
          enough contrast, especially on bright posters. */}
      <div
        aria-hidden
      ...
      <span
        ...
        aria-hidden
      >
        <span className="w-12 h-12 rounded-full bg-black/45 backdrop-blur-sm flex items-center justify-center ring-1 ring-white/20">
      ...
      {/* HUGE rank numeral, bottom-left, gold/silver/bronze. Sits above
          the bottom info row so the numeral is the loudest thing on
          the card. */}
      <span
        aria-hidden
        className={`absolute left-3 bottom-1 leading-none font-black select-none pointer-events-none ${color} ${glow}`}
        style={{
      ...
      {/* Info row - right of the numeral so the numeral owns the left
          slab and the text has its own column. */}
      <div className="absolute inset-x-0 bottom-0 p-3 pl-[92px] md:pl-[112px] pointer-events-none">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-white text-[15px] font-bold truncate leading-tight">{name}</div>
            {secondary && (
