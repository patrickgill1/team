...
final class WatchGameModel: NSObject, ObservableObject, WCSessionDelegate {
    ...
    func liveClockSeconds() -> Int {
        ...
        if s.isLive, let started = s.clockStartedAtMs {
            let elapsed = max(0, Int((now.timeIntervalSince1970 * 1000 - started) / 1000))
            return s.clockOffsetSeconds + elapsed
        }
        return s.clockOffsetSeconds
    }

    func secondsUntilSub() -> Int? {
        guard let s = session, s.bellEnabled, let shift = s.shiftSeconds, shift > 0 else { return nil }
        let last = s.lastBellAtSec ?? 0
...
extension WatchGameSession {
    static func from(_ raw: [String: Any]) -> WatchGameSession {
        ...
        let period: String
        if let p = raw["period"] { period = String(describing: p) } else { period = "1" }
