...
const EmptyState: React.FC<Props> = ({ icon, title, description, cta, tone = 'light', compact }) => {
  ...
  const iconTile = isDark
    ...
    : 'bg-cyan-50 ring-1 ring-cyan-100 text-cyan-600';
      ...
      {cta && (
        <div className="mt-4">
          {cta.to ? (
            <Link
              to={cta.to}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-bold transition"
            >
              {cta.label} →
            </Link>
          ) : (
