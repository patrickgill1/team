...
const NotificationPreferences: React.FC = () => {
  ...
  // Diagnostic state — restored 2026-08-04 gated on isClubAdmin so
  // it stays a Patrick-only support tool without cluttering the
  // profile for parents. When iOS pushes stop delivering (or
  // anyone reports "I'm not getting notifications"), this is the
  // fastest self-serve check for token registration + FCM health.
  const isAdmin = (userData as any)?.isClubAdmin === true;
  ...
  const sendTest = async () => {
    ...
    try {
      ...
      // If the worker pruned invalid tokens, refresh the visible count
      // so the admin sees the list shrink after a test send.
      if (Array.isArray(data?.invalidTokens) && data.invalidTokens.length > 0) {
        try {
          const fresh = await getDoc(doc(db, 'users', userData.uid));
          const arr = Array.isArray(fresh.data()?.fcmTokens) ? fresh.data().fcmTokens : [];
          setDiag(d => ({ ...d, tokenCount: arr.length }));
        } catch { /* ignore */ }
      }
    } catch (err: any) {
  ...
  return (
    <div className="bg-surface-elevated rounded-xl ring-1 ring-line-default/10 shadow-sm overflow-hidden">
      ...
      {/* Admin-only delivery test. Gated on isClubAdmin so this
          diagnostic doesn't clutter the parent-facing profile —
          Patrick + club admins get a self-serve "am I actually
          reachable?" check without needing a server-side script.
          Also surfaces the pruned-token count so it's obvious the
          worker is auto-cleaning FCM-dead tokens on send. */}
      {isAdmin && (
        <div className="px-4 py-3 border-b border-line-default/5 bg-surface-base/40">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="text-[10px] font-extrabold tracking-widest uppercase text-ink-primary/50">Admin: delivery test</div>
          ...
          {diag.response && (
            <div className={`mt-2 rounded-lg px-3 py-2 text-[11px] ring-1 ${
              ...
              {diag.response.invalidCount > 0 && (
                <div className="mt-1 opacity-80">
                  Pruned {diag.response.invalidCount} stale token{diag.response.invalidCount === 1 ? '' : 's'} — old install or app reinstall. List above should reflect the smaller count.
                </div>
              )}
            </div>
          )}
