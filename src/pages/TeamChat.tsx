              ...
              {membersSheetData.map((m) => (
                <li key={m.uid} className="px-4 py-2 flex items-center gap-3">
                  {m.photoUrl ? (
                    ...
                    <span
                      aria-hidden
                      className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 bg-slate-500 theme-ok"
                    >
  ...
  const lightbox = lightboxUrl ? (
    ...
    >
      <button
        ...
        className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/15 hover:bg-white/25 text-white flex items-center justify-center theme-ok"
