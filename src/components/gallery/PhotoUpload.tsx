...
const PhotoUpload: React.FC<PhotoUploadProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900/80 rounded-lg max-w-4xl w-full max-h-screen overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-gray-900/95 backdrop-blur border-b border-white/10 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-gray-400 hover:text-gray-300 transition-colors duration-200 disabled:opacity-50"
            >
        ...
        <div className="p-6">
          ...
          {!selectedFiles.length && (
            <div className="border-2 border-dashed border-white/15 rounded-lg p-8 text-center">
              <input
                type="file"
                multiple
                accept="image/*"
              ...
              >
                ...
                <span className="text-lg font-medium text-white mb-2">Select Photos</span>
                <span className="text-sm text-gray-400">Choose up to 10 images (JPG, PNG, GIF)</span>
              </label>
            </div>
          )}
          ...
          {selectedFiles.length > 0 && (
            <div className="space-y-6">
                  ...
                  return (
                    <div key={index} className="relative bg-gray-100 rounded-lg overflow-hidden">
                      ...
                      <button
                        ...
                        disabled={isUploading || uploadLoading}
                        className="absolute top-2 right-2 bg-rose-500/100 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 transition-colors duration-200 disabled:opacity-50"
                      >
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-white">Add Captions</h3>
                {selectedFiles.map((file, index) => (
                  <div key={file.name}>
                    ...
                    <input
                      ...
                      disabled={isUploading || uploadLoading}
                      className="w-full px-3 py-2 border border-white/15 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
                    />
                  </div>
                ))}
              </div>
              ...
              {selectedFiles.length < 10 && (
                <div>
                  ...
                  <label
                    ...
                    className="inline-flex items-center px-4 py-2 border border-white/15 rounded-lg text-sm font-medium text-gray-200 bg-white/10 hover:bg-white/15 transition-colors duration-200 cursor-pointer"
                  >
          ...
          {/* Error Message */}
          {(error || uploadError) && (
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-lg p-3 mt-4">
              <p className="text-rose-300 text-sm">{error || uploadError}</p>
            </div>
          )}
          ...
          {/* Action Buttons */}
          {selectedFiles.length > 0 && (
            <div className="flex space-x-4 pt-6 border-t border-white/10 mt-6">
              <button
                type="button"
                onClick={onClose}
                disabled={isUploading || uploadLoading}
