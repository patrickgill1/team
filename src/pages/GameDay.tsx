...
import { isCoachOfTeam, isStaffOfTeam, isOwner, resolveSenderRole } from '../utils/helpers';
...
interface LiveGameDoc {
   ...
   *  fallback '7v7'. */
  format?: '4v4' | '5v5' | '6v6' | '7v7' | '8v8' | '9v9' | '10v10' | '11v11';
  ourScore: number;
  oppScore: number;
...
const KIND_META: Record<StatKind, { label: string; emoji: string; color: string }> = {
  ...
  note:    { label: 'Note theme-ok',     emoji: '📝', color: 'bg-gray-500' },
};
...
const GameDay: React.FC = () => {
  ...
  const ensureGameDoc = async (): Promise<LiveGameDoc> => {
    ...
    let format: '4v4' | '5v5' | '6v6' | '7v7' | '8v8' | '9v9' | '10v10' | '11v11' = '7v7';
    try {
      const teamDoc = await getDoc(doc(db, 'teams', event.teamId));
      const f = (teamDoc.exists() && (teamDoc.data() as any).format) || '7v7';
      if (['4v4', '5v5', '6v6', '7v7', '8v8', '9v9', '10v10', '11v11'].includes(f)) format = f;
    } catch {}
      ...
      {pendingSubUndo && (
        <div className="sticky top-0 z-30 bg-emerald-600 text-white px-4 py-2 flex items-center justify-between gap-3 text-sm font-bold shadow-lg animate-slide-down">
          ...
          <button
            ...
            className="text-[11px] tracking-widest uppercase font-black bg-white/20 rounded-full px-3 py-1 hover:bg-white/30 active:bg-white/40 transition-colors theme-ok"
          >
            Undo
          </button>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              ...
              className="text-[11px] tracking-widest uppercase font-black bg-white/10 rounded-full px-3 py-1 hover:bg-white/25 active:bg-white/40 transition-colors theme-ok"
              aria-label="Dismiss"
            >
              ×
            </button>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-bold ${
                ...
                status === 'final theme-ok' ? 'bg-gray-500/20 text-ink-primary/45 ring-1 ring-gray-500/40' :
        ...
        {status === 'final' && isAdultTeam && isUserCoach && (
          <div className="mt-3 flex justify-end">
            <button
              ...
              className="px-4 py-2.5 rounded-full bg-brand-primary text-white text-xs font-extrabold uppercase tracking-widest hover:bg-brand-primary-soft hover:text-ink-primary transition"
            >
        ...
        {status !== 'final' && lineup.onField.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-2">
              ...
              {isUserCoach && (
                <div className="flex flex-wrap gap-0.5 bg-line-default/5 ring-1 ring-line-default/15 rounded-xl p-0.5 justify-end max-w-[240px]">
                      ...
                      .map(pid => {
                        ...
                        return (
                          <button
                            ...
                            title="Tap to sub ON"
                          >
                            <span className={`w-7 h-7 rounded-full text-white text-[10px] font-black flex items-center justify-center flex-shrink-0 ${isNext ? 'bg-amber-600 theme-ok' : 'bg-slate-600'}`}>
                              {p.jerseyNumber != null ? `#${p.jerseyNumber}` : (p.name || '?').charAt(0)}
                            </span>
                            <span className="flex-1 min-w-0">
