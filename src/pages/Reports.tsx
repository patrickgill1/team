            ...
            <Section title="By age group">
              {byAge.length === 0
                ...
                : <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {byAge.map(([k, n]) => (
                      <div key={k} className="rounded-lg ring-1 ring-slate-200 bg-slate-50 p-2 text-center">
                        <div className="text-lg font-black text-slate-900">{n}</div>
                        <div className="text-[10px] font-extrabold uppercase tracking-widest text-slate-500">{k}</div>
                      </div>
                    ))}
                  </div>}
            </Section>
                ...
                : <ul className="space-y-1">
                    {byStatus.map(([status, n]) => (
                      <li key={status} className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-bold text-slate-900 tabular-nums">{n}</span>
                      </li>
                    ))}
                  </ul>}
            </Section>
                ...
                : <ul className="space-y-1">
                    {coupons.map(([code, info]) => (
                      <li key={code} className="flex items-center justify-between text-sm">
                        <span className="font-bold text-slate-900 tracking-wider">{code}</span>
                        <span className="text-slate-600 tabular-nums">{info.count}× · {fmtCents(info.discountCents)} off</span>
                      </li>
                    ))}
                  </ul>}
            </Section>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="bg-white rounded-2xl ring-1 ring-gray-200 overflow-hidden">
    <div className="px-4 py-2 border-b border-slate-100">
      <h2 className="text-[10px] font-extrabold uppercase tracking-widest text-slate-600">{title}</h2>
    </div>
  ...
  return (
    <div className="mb-2 last:mb-0">
      <div className="flex items-baseline justify-between text-xs">
        ...
        <span className="text-slate-600 tabular-nums">{value} <span className="text-slate-400">({pct}%)</span></span>
      </div>
