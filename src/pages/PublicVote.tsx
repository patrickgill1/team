...
const AnimatedBar: React.FC<{ percentage: number; isLeader: boolean }> = ({ percentage, isLeader }) => {
  ...
  return (
    <div className="flex-1 bg-line-default/[0.08] rounded-full h-2 overflow-hidden">
      <div
...
const PositionBadge: React.FC<{ position?: string }> = ({ position }) => {
  ...
  const colour = colours[position] || 'bg-line-default/[0.04] text-ink-primary/65 border border-line-default/10';
  return (
...
const ConfettiTest: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] flex items-center justify-center p-4">
      ...
      <div className="bg-surface-elevated rounded-2xl shadow-lg p-8 max-w-sm w-full text-center border border-line-default/5 relative z-10">
        <div className="w-14 h-14 rounded-full bg-[#f0f9ff] flex items-center justify-center mx-auto mb-4">
          <span className="text-2xl">🎉</span>
        </div>
        <p className="text-xs font-bold text-ink-primary/40 uppercase tracking-widest mb-1">GoalKickr</p>
        <h1 className="text-xl font-black text-ink-primary mb-2">Confetti test</h1>
        <p className="text-sm text-ink-primary/50 mb-6">
...
const PublicVote: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-[#f0f4f8] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-ink-primary/40 font-medium text-sm">Loading…</p>
        </div>
      </div>
    );
  }
  ...
  if (error && !voting) {
    return (
      <div className="min-h-screen bg-[#f0f4f8] flex items-center justify-center p-4">
        <div className="bg-surface-elevated rounded-2xl shadow-lg p-8 max-w-sm w-full text-center border border-line-default/5">
          <div className="w-14 h-14 rounded-full bg-[#f0f9ff] flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl">🔥</span>
          </div>
          <p className="text-xs font-bold text-ink-primary/40 uppercase tracking-widest mb-1">GoalKickr</p>
          <h1 className="text-xl font-black text-ink-primary">Player of the Match</h1>
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f0f4f8] flex items-start justify-center p-4 pt-8 pb-16">
        <div className="bg-surface-elevated rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-line-default/5">
          ...
          <div className="bg-black px-6 pt-8 pb-6 text-center relative overflow-hidden">
            ...
            <div className="relative">
              ...
              {voting && (
                <>
                  <p className="text-ink-primary/35 font-semibold mt-2 text-base">{voting.gameTitle}</p>
                  <p className="text-ink-primary/50 text-sm mt-0.5">{formatDate(voting.gameDate)}</p>
                  <div className="flex flex-wrap justify-center gap-2 mt-4">
                    ...
                    {voting.opponent && (
                      <span className="bg-surface-elevated bg-opacity-10 border border-white border-opacity-20 text-ink-primary/35 text-xs font-semibold px-3 py-1 rounded-full">
          ...
          <div className="p-6">
            ...
            {voting && !voting.isActive && (
              <div className="mb-5 rounded-xl overflow-hidden border border-line-default/10">
                <div className="bg-line-default/[0.04] px-4 py-3 text-center">
                  <p className="text-ink-primary/40 font-bold text-xs uppercase tracking-widest">Voting Closed</p>
                </div>
                  ...
                  <div className="bg-[#f0f9ff] px-4 py-5 text-center border-t border-[#159BE3] border-opacity-20">
                    {voting.winners && voting.winners.length > 1 ? (
                      <>
                        ...
                        <p className="text-xl font-black text-ink-primary">🏆 {voting.winners.map(w => w.playerName).join(' · ')}</p>
                        <p className="text-ink-primary/50 text-sm mt-1">{voting.winners[0].voteCount} vote{voting.winners[0].voteCount !== 1 ? 's' : ''} each</p>
                      </>
                    ) : (
                      <>
                        <p className="text-[#159BE3] text-xs font-black uppercase tracking-widest mb-1">Winner</p>
                        <p className="text-2xl font-black text-ink-primary">🏆 {(voting.winners?.[0] || voting.winner)!.playerName}</p>
                        <p className="text-ink-primary/50 text-sm mt-1">{(voting.winners?.[0] || voting.winner)!.voteCount} vote{(voting.winners?.[0] || voting.winner)!.voteCount !== 1 ? 's' : ''}</p>
                      </>
                    )}
                  </div>
                )}
            ...
            {alreadyVoted && voting?.isActive && !submitted && (
              <div className="bg-[#f0f9ff] border border-[#159BE3] border-opacity-30 rounded-xl p-4 mb-5 text-center">
                ...
                <p className="font-bold text-ink-primary">You've already voted!</p>
                <p className="text-ink-primary/50 text-sm mt-1">Your vote is locked in. Results will be revealed once voting closes.</p>
              </div>
            )}
            ...
            {submitted && (
              <div className="bg-[#f0f9ff] border border-[#159BE3] border-opacity-40 rounded-xl p-5 mb-5 text-center">
                ...
                <p className="font-black text-ink-primary text-lg">Vote submitted!</p>
                <p className="text-ink-primary/50 text-sm mt-1">Thanks, <strong>{voterName}</strong>! Results will be revealed once voting closes.</p>
              </div>
            )}
            ...
            {step === 'identify' && voting?.isActive && (
              <div className="space-y-5">
                <div>
                  <h2 className="text-lg font-bold text-ink-primary">Before you vote…</h2>
                  <p className="text-ink-primary/40 text-sm">Two quick things and you're in.</p>
                </div>

                <div>
                  <label className="block text-xs font-bold text-ink-primary/40 uppercase tracking-widest mb-1.5">
                  ...
                  <input
                    ...
                    className="w-full border border-line-default/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#159BE3] focus:border-transparent text-sm transition-shadow"
                  />
                </div>

                <label className="flex items-start gap-2.5 p-3 rounded-xl border-2 border-line-default/10 hover:border-[#159BE3] hover:border-opacity-40 cursor-pointer select-none transition-colors">
                  <input
                    type="checkbox"
                    checked={iAmCoach}
                    onChange={e => { setIAmCoach(e.target.checked); if (e.target.checked) { setMyChildId(''); } }}
                    className="mt-0.5 w-4 h-4 rounded border-line-default/15 text-[#159BE3] focus:ring-[#159BE3]"
                  />
                  <div className="flex-1 text-sm">
                    <div className="font-bold text-ink-primary">🧥 I'm a coach (no child on the team)</div>
                    <div className="text-xs text-ink-primary/50 mt-0.5">Skips the parent check so you can vote for any player.</div>
                  </div>
                </label>
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-primary/40 mb-2">They'll be excluded from your choices — keeping it fair.</p>
                        ...
                        <button
                          ...
                          className={`w-full flex items-center gap-3 p-2.5 rounded-xl border-2 transition-all duration-150 text-left ${
                            isSelected
                              ...
                              : 'border-line-default/10 hover:border-[#159BE3] hover:border-opacity-40 hover:bg-line-default/[0.05]'
                          }`}
                        >
                          {p.profilePhotoUrl ? (
                            <img
                              ...
                              alt={p.name}
                              className={`w-11 h-11 rounded-full object-cover flex-shrink-0 border-2 ${isSelected ? 'border-[#159BE3]' : 'border-line-default/5'}`}
                            />
                          ) : (
                            <div className={`w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${isSelected ? 'bg-[#159BE3]' : 'bg-line-default/15'}`}>
                              <span className={`font-bold text-base ${isSelected ? 'text-white' : 'text-ink-primary/50'}`}>
                                {p.name.charAt(0).toUpperCase()}
                              </span>
                            </div>
                          )}
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-ink-primary text-sm truncate">{p.name}</p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              {p.jerseyNumber && (
                                <span className="text-xs text-ink-primary/50 bg-line-default/[0.08] px-1.5 py-0.5 rounded-full font-mono">
                                  #{p.jerseyNumber}
                                </span>
                              )}
            ...
            {step === 'vote' && voting?.isActive && (
              <div className="space-y-5">
                <div>
                  <h2 className="text-lg font-bold text-ink-primary">Cast your vote</h2>
                  <p className="text-ink-primary/40 text-sm">
                      ...
                      >
                        ...
                        {player.profilePhotoUrl ? (
                          <img
                            ...
                            alt={player.name}
                            className={`w-16 h-16 rounded-full object-cover mb-2 border-2 shadow-sm ${isSelected ? 'border-[#159BE3]' : 'border-line-default/5'}`}
                          />
                        ) : (
                        ...
                        <span className="font-bold text-ink-primary text-sm text-center leading-tight w-full">{player.name}</span>
                ...
                {selectedPlayerId && (
                  <div>
                    <label className="block text-xs font-bold text-ink-primary/40 uppercase tracking-widest mb-1.5">
                      Why did they stand out? <span className="text-ink-primary/40 font-normal normal-case tracking-normal">(optional)</span>
                    </label>
                    <textarea
                      value={voteReason}
                      ...
                      className="w-full border border-line-default/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#159BE3] focus:border-transparent text-sm resize-none transition-shadow"
                    />
                  </div>
                )}
                ...
                <div className="flex gap-3 pt-1">
                  <button
                    ...
                    className="px-4 py-3 rounded-xl border border-line-default/10 text-ink-primary/65 hover:bg-line-default/[0.05] transition-colors text-sm font-semibold"
                  >
                    ← Back
                  </button>
                  <button
            ...
            {step === 'results' && voting?.isActive && (
              <div className="bg-line-default/[0.04] border border-line-default/10 rounded-xl p-6 text-center">
                <svg className="w-10 h-10 text-ink-primary/40 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21" />
                </svg>
                <p className="font-bold text-ink-primary/85">Results are hidden while voting is open</p>
                <p className="text-ink-primary/50 text-sm mt-1">Check back once the coach closes voting to see the results!</p>
                <p className="text-ink-primary/40 text-xs mt-3">{totalVotes} vote{totalVotes !== 1 ? 's' : ''} cast so far</p>
              </div>
            )}
            ...
            {step === 'results' && !voting?.isActive && (
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-bold text-ink-primary">Final Results</h2>
                </div>

                {results.length === 0 ? (
                  <div className="text-center py-12 text-ink-primary/40">
                        ...
                        <div
                          ...
                          className={`rounded-xl border overflow-hidden ${
                            isLeader
                              ...
                              : 'border-line-default/10'
                          }`}
                        >
                          <div className={`p-3.5 ${isLeader ? 'bg-[#f0f9ff]' : 'bg-surface-elevated'}`}>
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <p className="font-bold text-ink-primary truncate text-sm">{result.name}</p>
                                  {result.position && <PositionBadge position={result.position} />}
                                </div>
                                <div className="flex items-center gap-2 mt-1.5">
                                  <AnimatedBar percentage={percentage} isLeader={isLeader} />
                                  <span className="text-xs font-semibold whitespace-nowrap text-ink-primary/50">
                          ...
                          {reasons.length > 0 && (
                            <div className={`border-t px-3.5 py-2.5 space-y-1.5 ${
                              isLeader ? 'border-[#159BE3] border-opacity-20 bg-[#f7fbff]' : 'border-line-default/5 bg-line-default/[0.04]'
                            }`}>
                              {reasons.map((v, i) => (
                                <p key={i} className="text-xs text-ink-primary/65 flex gap-1.5">
                                  <span className="flex-shrink-0 text-[#159BE3] font-bold">›</span>
                                  <span>
                                    <span className="font-semibold text-ink-primary/85">{v.voterName}:</span> {v.reason}
                                  </span>
                                </p>
                              ))}
                            </div>
                ...
                <p className="text-center text-ink-primary/40 text-xs mt-5">
          ...
          {/* Footer */}
          <div className="border-t border-line-default/5 px-6 py-3 flex items-center justify-center gap-2">
            <span className="text-xs text-ink-primary/40 font-bold tracking-widest uppercase">GoalKickr</span>
          </div>
        </div>
      </div>
    </>
