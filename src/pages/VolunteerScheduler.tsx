      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">Volunteer Scheduler</h1>
              <p className="text-gray-300 mt-1">Help make team events successful by volunteering</p>
            </div>
            <button
          ...
          {upcomingEvents.length === 0 && (
            <div className="mt-4 p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-amber-200 font-medium">No upcoming events found</p>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {upcomingEvents.map(event => {
                ...
                return (
                  <div key={event.id} className="bg-gray-900/80 rounded-lg border border-white/10 p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-semibold text-white">{event.title}</h3>
                        <p className="text-sm text-gray-300">{formatDateTime(event.date)}</p>
                    ...
                    {eventOpps.length > 0 ? (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-300">Volunteer needs:</span>
                          <span className={`font-medium ${
                            filledSlots >= totalSlots ? 'text-emerald-300' : 'text-orange-300'
                          }`}>
                            {filledSlots}/{totalSlots} filled
                          </span>
                        </div>
                        {eventOpps.map(opp => (
                          <div key={opp.id} className="text-xs text-gray-300 flex items-center justify-between">
                      ...
                      <p className="text-sm text-gray-400">No volunteer needs yet</p>
                    )}
                  </div>
                );
              })}
              ...
              <div key={opportunity.id} className="card-modern border border-white/10 overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex items-center space-x-2 text-sm">
                    ...
                    <span className={slotsRemaining > 0 ? 'text-gray-300' : 'text-rose-300'}>
                  ...
                  {opportunity.slotsBooked.length > 0 && (
                    <div className="pt-3 border-t border-white/10">
                      <h4 className="text-sm font-medium text-gray-200 mb-2">Volunteers:</h4>
                ...
                <div className="p-4 border-t border-white/10">
                  {isSignedUp ? (
                    <button
                      ...
                      className="w-full bg-red-100 hover:bg-red-200 text-rose-300 font-medium py-2 px-4 rounded-lg transition duration-200"
                    >
                      Withdraw
                    </button>
                  ) : (
        ...
        {filteredOpportunities.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-white mb-2">No volunteer opportunities</h3>
            <p className="text-gray-300">Create volunteer needs for your team events</p>
          </div>
        )}
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900/80 rounded-lg max-w-md w-full p-6">
            <h2 className="text-xl font-semibold text-white mb-4">Add Volunteer Need</h2>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-200 mb-1">For which event?</label>
                <select
                  value={selectedEvent}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-200 mb-1">Type of help needed</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-200 mb-1">Volunteers needed</label>
                  <input
                    type="number"
                    min="1"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-200 mb-1">Time offset (minutes)</label>
                  <input
                    type="number"
