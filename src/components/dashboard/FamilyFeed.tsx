...
const FamilyFeed: React.FC<Props> = ({ hidden }) => {
  ...
  // Collapsed shape: every other team is hidden. Just show a tiny
  // one-liner at the bottom of the Dashboard instead of a big empty
  // block that reads as "something's broken."
  if (visibleTeams.length === 0 && hiddenCount > 0 && !showHidden) {
    return (
      <section className="mt-3 mb-4">
        <button
          type="button"
          onClick={() => setShowHidden(true)}
          className="w-full flex items-center gap-2 py-2 text-[11px] text-ink-primary/45 hover:text-ink-primary/85 transition-colors"
        >
          ...
          <span>{hiddenCount} {teamWord(hiddenCount)} hidden from Home</span>
          <span className="ml-auto text-brand-primary-soft font-bold">Manage</span>
        </button>
      </section>
    );
  }
