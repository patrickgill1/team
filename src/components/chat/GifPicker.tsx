...
const GifPicker: React.FC<Props> = ({ isOpen, onClose, onPick }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col"
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {loading && results.length === 0 ? (
                ...
                <div className="p-6 text-center text-sm text-gray-400">No GIFs found.</div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                  {results.map((g) => (
                    <button
                      ...
                      className="relative overflow-hidden rounded-lg bg-gray-100 hover:opacity-80 transition-opacity active:scale-95"
